...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    // Scrub-out: hero content lifts and fades as the journey begins.
  ...
  return (
    <section
      ref={sectionRef}
      className="relative flex min-h-[82vh] items-center pt-6 lg:pt-10"
    ...
    >
      ...
      {/* Floating badges over the 3D object (desktop) */}
      <motion.div
        animate={{ y: [0, -6, 0] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
        className="hidden lg:block absolute top-[16%] right-[6%] z-20 bg-gradient-to-r from-primary-500 to-secondary-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg shadow-primary-900/40 pointer-events-none"
      >
        Open to work
      </motion.div>
      <motion.div
        ...
        className="hidden lg:inline-flex absolute bottom-[18%] right-[24%] z-20 bg-[#1a1a1a] border border-white/15 text-white text-xs font-medium px-3 py-1.5 rounded-full shadow-lg items-center gap-1.5 pointer-events-none"
      >
      ...
      {/* Scroll hint */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 hidden sm:flex flex-col items-center gap-1.5 text-slate-500 pointer-events-none">
        <span className="text-[10px] tracking-[0.25em] uppercase">Scroll</span>
        <span className="block w-px h-8 bg-gradient-to-b from-slate-500 to-transparent" />
      </div>
    </section>
  );
};
